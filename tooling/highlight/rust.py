"""
    ...
    Extension of the default Rust lexer from the Pygments team.

    Includes:
    - Generic param highlighting
...
from pygments.token import Text, Comment, Operator, Keyword, Name, String, \
    Number, Punctuation, Whitespace, Generic, Token
...
def generic_params(lexer: RegexLexer, match: re.Match) -> Iterator[tuple[int, Token, str]]:
    GENERIC_PARAMS.add(match.string)
    yield match.start(), Generic, match.string
...
class RustLexer(RegexLexer):
    ...
    filenames: ClassVar[list[str]] = ['*.rs', '*.rs.in']
    aliases: ClassVar[list[str]] = ['rust', 'rs']
    mimetypes: ClassVar[list[str]] = ['text/rust', 'text/x-rust']
    ...
    tokens: ClassVar[dict[str, ...]] = {
        ...
        'funcname': [
            (r'\s+', Text),
            (r'[a-zA-Z_]\w*', Name.Function, 'param_spec'),
            default('#pop'),
        ],
        'param_spec': [
            (r'<', Text, "type_params"),
        ...
        'type_params': [
            (r'\s+', Text),
            (r'[a-zA-Z_]\w*', generic_params),
            (r">", "#pop"),
        ],
        "params": [
            (r'\s+', Text),
            (r'[a-zA-Z_]\w*:', Text, "param_types"),
            (r")", "#pop"),
        ],
        "param_types": [
            (r'\s+', Text),
